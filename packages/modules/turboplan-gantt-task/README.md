# @wildfires-org/turboplan-gantt-task

Interactive Gantt chart for React with TypeScript. Vendored fork of [gantt-task-react](https://github.com/MaTeMaTuK/gantt-task-react) (MIT), updated for React 19 and maintained inside the TurboPlan monorepo. Used by `@wildfires-org/turboplan-tasks` for its Gantt view.

## Install

Internal workspace package — add it as a dependency:

```json
{
  "dependencies": {
    "@wildfires-org/turboplan-gantt-task": "workspace:*"
  }
}
```
...
- Root — `Gantt` component, `ViewMode` enum, `useTaskStyles` hook, types (`Task`, `GanttProps`, `EventOption`, `DisplayOption`, `StylingOption`, `TaskStyles`, `TaskTypeExternal`)
- `./index.css` (alias `./style.css`) — required stylesheet

## How to use it

```typescript
import { Gantt, Task, EventOption, StylingOption, ViewMode, DisplayOption } from "@wildfires-org/turboplan-gantt-task";
...
| preStepsCount  | boolean | Adds extra empty space before the first task (default `true`).                                              |
