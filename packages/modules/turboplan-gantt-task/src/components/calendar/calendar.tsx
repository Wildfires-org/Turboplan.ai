  ...
  const getCalendarValuesForYear = () => {
    ...
    for (let i = 0; i < dateSetup.dates.length; i++) {
      ...
      for (let month = 0; month < 12; month++) {
        const dummyDate = new Date(date.getFullYear(), month, 1);
        const bottomValue = getLocaleMonth(dummyDate, locale, "narrow");
        bottomValues.push(
          <text
            key={dummyDate.getTime()}
            y={headerHeight * 0.8}
            x={
              columnWidth * i +
              ((columnWidth / 12) * month + (columnWidth / 12) * 0.5)
            }
